...
> ## ⛔ Инстанс `events-dev` очищен 13.09.2026
>
> В проекте **0 флоу и 0 таблиц** (`ap_list_flows`, `ap_list_tables`); выжил
> connection `AI Qadam Events (dev)`. Пакеты ниже сохраняют статус `готов` —
> он означает **«работа сделана и принята ревью»**, а не «артефакт существует».
> Живое состояние проекта — всегда
> [catalog/overview.md](../catalog/overview.md), не этот файл.
>
> Пересборка по [ADR-0015](adr/0015-one-touch-one-flow.md) — пакет
> [W26](work/W26-rebuild-on-one-touch.md). До его закрытия считать построенным
> **ничего** нельзя, включая то, что помечено `готов`.
...
| W11. Визард ивента | 3 | W2, W3, [Q36](OPEN-QUESTIONS.md#q36) | не начат — [был собран мимо процесса и удалён вместе с инстансом](#визард-ивента-был-собран-мимо-процесса) | — | — |
...
| W26. Пересборка `events-dev` по [ADR-0015](adr/0015-one-touch-one-flow.md) | 5 | ADR-0015, [Q35](OPEN-QUESTIONS.md#q35) | **в работе** | агент W26 | [W26](work/W26-rebuild-on-one-touch.md) |
| W15. Приёмка | 6 | все, включая W26 | не начат | — | — |

Критический путь после очистки инстанса: `Variables → W26 (таблицы → Q35 → fn-* → tg-router → касания) → W11 → W15`.
...
- **Решения.** ADR-0001 (Qadam Flow как несущий стек, «0 кода»), ADR-0002 (роли таблицами),
  ...
  сужает 0012, возвращает `fn-*` для крипты и общих чтений)**. Полный индекс —
  [adr/README.md](adr/README.md).
- **Открытые вопросы.** Полный список и статусы — [OPEN-QUESTIONS.md](OPEN-QUESTIONS.md);
  счётчик здесь сознательно не дублируется, он разъезжается с каждым новым вопросом.
  Заведены 2026-09-13: [Q35](OPEN-QUESTIONS.md#q35) (цена `callFlow` после
  апстрим-фиксов — блокирует `fn-*` в W26) и [Q36](OPEN-QUESTIONS.md#q36)
  (рассылка по списку не касание и не функция — дыра в ADR-0015, блокирует W11).

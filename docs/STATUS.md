...
> ## Куда идти дальше (2026-09-15, v0.1/v0.2 — решение владельца)
>
> Скоуп переписан: **v0.1 = три идеальных опыта** (гость: ссылка → регистрация
> → QR; овнер: `/start` → меню → Mini App → ивент → ссылка-приглашение;
> контролёр: открыл сканер → зачекинил). Опорные пакеты закрыты (W28, W30,
> W33, W34) — v0.1 это четыре маленьких пакета и приёмка, а не пересборка.
> **W41** (UI-прототипы end-to-end — дизайн-гейт, перенесён в v0.1 решением
> владельца 2026-09-15, идёт перед приёмкой W15) — всё остальное — v0.2:
> W13 (списки/экспорт), W14 (рассылки, headline-UX —
...
> `telegram_id` в `#/manage/:id`) → **W40** (команды, кроме `/start`, убраны —
> [ADR-0025](adr/0025-start-only-commands-ban.md), указание владельца «сразу»)
> → **W37** (полировка трёх путей: ссылка после создания, список своих
> ивентов в `#/manage` — он и возвращает вход в правку, замеры ADR-0009) →
> **W41** (дизайн-гейт: UI-прототипы end-to-end, вердикт владельца по
> экранам до приёмки) → **W15** (приёмка v0.1; сценарий «прерванная
> рассылка» не применим).
> W36 и W37 оба правят `manage`-часть — строго последовательно; W40 — первый
> пакет v0.1, трогающий `tg-router`, идёт сразу после W36. Порядок и обоснование —
...
| W12. Lifecycle и напоминания | 4 | W11 | не начат — **v0.2** (2026-09-15) | — | — |
| W7. Mini App — сканер | 5 | W8, 0.4–0.5 | **готов** 2026-09-09 | агент W7 | [W07](work/W07-miniapp-scanner.md) |
| W13. Списки и экспорт | 5 | W5 | не начат — **v0.2** (2026-09-15) | — | — |
| W14. Рассылки | 5 | W13, W3 | не начат — **v0.2** (2026-09-15); headline-UX «переслал боту сообщение → разослал» — в секции пакета | — | требует [Q13](OPEN-QUESTIONS.md#q13) |
...
| W42. Визард ивента: шаги, гео ссылкой и точкой на карте (вердикт W41) | v0.2 | W33 — ✅, W37 — ✅, [Q52](OPEN-QUESTIONS.md#q52) — для карты | не начат — **берётся в v0.2**, заведён по вердикту 2026-09-16 | — | — |
| W43. Регистрация и «Мои билеты» в каталоге `#/events` (вердикт W41) | v0.2 | [W38](BACKLOG.md#w38-четвёртая-страница-mini-app--каталог-ивентов-events), W28 — ✅ | не начат — **берётся в v0.2**, заведён по вердикту 2026-09-16 | — | — |
| W44. Контролёры: выбор из списка с поиском (вердикт W41) | v0.2 | W36 — ✅, [Q51](OPEN-QUESTIONS.md#q51) | не начат — **берётся в v0.2**, заведён по вердикту 2026-09-16 | — | — |
| W45. Послесловие: благодарность и форма отзыва (вердикт W41) | v0.2 | [ADR-0028](adr/0028-feedback-screen-fifth-miniapp-page.md) (черновик), W12, [Q53](OPEN-QUESTIONS.md#q53) | не начат — **берётся в v0.2**, заведён по вердикту 2026-09-16 | — | — |
...
Критический путь v0.1: **`W35 → W32 → W36 → W40 → W37 → W41 → W15`** —
W41 закрыт 2026-09-16 (дизайн-гейт пройден, вердикт владельца и независимое
ревью), следующий и последний — **W15 (приёмка)**.
W28, W30, W33, W34 пройдены и составляют построенную часть v0.1 (обоснование
порядка — [ROADMAP.md](ROADMAP.md#v01--порядок-работ-один-агент-последовательно)).
W36 и W37 оба правят `manage`-часть — строго последовательно, между ними W40.
W40 убирает команды, кроме `/start` ([ADR-0025](adr/0025-start-only-commands-ban.md)) —
первый пакет v0.1, трогающий `tg-router`, взят сразу после W36 по указанию
владельца; вход в правку ивента возвращает W37 списком `#/manage`. W41 —
дизайн-гейт перед приёмкой (перенесён в v0.1 решением владельца 2026-09-15,
инстанс не трогает): вердикт по экранам до приёмки W15. W13, W14,
W12, W10, W12b, W38 — v0.2 (решение владельца 2026-09-15). Полная картина —

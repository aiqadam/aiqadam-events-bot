...
> (owner'ы по списку, `initData` 5 минут; нужна первая строка `owners` —
> шаг человека, 0.8) → **W36** (staff-lite: права контролёра по
...
> W36 и W37 оба правят `manage`-часть — строго последовательно; `tg-router`
> в v0.1 не трогает ни один пакет. Порядок и обоснование —
...
| 0.8 | `telegram_id` владельца проекта — первая строка `owners` | **нужен до W32** | W32 |
...
| W36. Staff-lite — права контролёра по `telegram_id` в `manage` SPA | v0.1 | W32 | не начат — **берётся после W32** | — | — |
| W37. Полировка трёх путей v0.1 (замеры ADR-0009, ссылка-приглашение, список ивентов в `#/manage`) | v0.1 | W32, W36 | не начат — **берётся после W36** | — | — |
...
| W32. Owner'ы по списку, `initData` 5 минут, афиша снята ([Q46](OPEN-QUESTIONS.md#q46), [Q47](OPEN-QUESTIONS.md#q47)) | 3 | W33 — ✅ сдан | **свободен** — до W15: без него создать ивент вправе любой пользователь бота; нужен `telegram_id` владельца для `owners` (шаг человека) | — | — |
...
| W35. Дозапись `migrations` за W33–W34 ([ADR-0021](adr/0021-repo-is-source-of-truth-migrations-table.md)) | вне волн | W34 — ✅ закрыт (`251d3f1`) | **на проверке** 2026-09-15 — 12 строк вставлены, сверка `_manifest.json` ↔ `migrations` ↔ `ap_list_flows` пройдена; ждёт независимого ревью | агент | [W35](work/W35-migrations-backfill.md) |
| W15. Приёмка | 9 | все `готов` на момент приёмки | не начат | — | — |

Критический путь v0.1 на 2026-09-15: **`W35 → W32 → W36 → W37 → W15`** —
...
W36 и W37 оба правят `manage`-часть — строго последовательно. W13, W14, W12,
W10, W12b — v0.2 (решение владельца 2026-09-15). Полная картина —
...
(записан в секции W14 BACKLOG). Ранние решения владельца 2026-09-14

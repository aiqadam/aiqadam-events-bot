...
Каждый флоу экспортируется в JSON и лежит в `flows/` — конфигурация продакшена
не должна существовать только в UI. Subflow-«функции» — в `flows/fn/`.
...
**T** `schedule` каждые 15 минут. Виды: `24h`, `2h`, `start`.
...
2. Цикл чанками, курсор в `broadcasts.cursor`:
   - **C** формирует пачку и выдерживает темп **≤ 25 msg/s** (OWN-11);
   - **A** `telegram sendMessage` с кнопкой **«Отписаться»** (OWN-13);
   - `429` → пауза ровно на `retry_after` из ответа, потом повтор того же адресата (OWN-12);
   - `403` → `users.blocked_bot = true`, target `blocked`, **больше никогда не дёргаем** (OWN-12);
   - каждый успех → `state = sent` немедленно, до перехода к следующему.
3. Чанк ограничен так, чтобы прогон не подходил к `FLOW_TIMEOUT_SECONDS = 600`;

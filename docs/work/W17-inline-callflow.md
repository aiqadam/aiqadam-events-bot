...
- `checkin-api` переведён на `inline`, весь `flowProps.payload` аудирован и
  переопубликован (см. журнал), но **ни один живой скан ещё не прогонялся** —
  нужен after-замер латентности на реальном `initData`/QR и повтор трёх
  различающих прогонов STF-2 + позитивного контроля.

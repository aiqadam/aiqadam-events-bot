...
- **2026-09-24 — готов.** Ревью 3 круга: круг 1 — блокеров и «важно» нет, два
  «на будущее» (сентинел `__none__` принят конвенцией и зарезервирован в
  `AGENTS.md`; устаревшая заметка о `demo`); круг 2 — одно «важно» (та же
  заметка осталась в `catalog/overview.md`, исправлено); круг 3 — «замечаний
  нет». Шесть флоу опубликованы, живой `curl` подтвердил `403`/`401` вместо
  `500`. Хвосты — в разделе ниже.
...
- **Вердикт**: есть замечания — блокеров нет, одно «важно»
  (исправление круга 1 неполно)

#### Ответы владельца на круг 1

1. **Замечание 1 (сентинел `__none__`) — снято (принято как конвенция).**
   `AGENTS.md` (раздел про fail-closed `eq` в `tables-find-records`)
   дополнен: «Сентинелы `__none__`/`-` — зарезервированные значения: реальный
   ключ не должен им равняться (id события `__none__` не заводить…)».
   Сверено живьём (`ap_find_records`): в `events` id — `mu9rqipgetmp` и
   `mu9uzchnu2il`, в `event_staff` — только `mu9rqipgetmp`; записи с ключом
   `__none__` нет. Вытеснение сентинела на не-слаговый (`-`) оставлено
   хвостом — как и решил владелец.
2. **Замечание 2 (устаревшая заметка о фикстуре `demo`) — закрыто частично.**
   Из `catalog/flows/checkin-api.md` абзац убран (дифф `76d25ff`, живой файл
   — строки 67–103, заметки нет). Позитивный различающий прогон STF-2
   вынесен в хвосты — обоснованно: в `event_staff` единственная активная
   строка — `mu9rqipgetmp`/`5895728710`, staff-записи на владельца
   (`322876545`) нет. **Но то же устаревшее утверждение осталось в
   `catalog/overview.md`** — см. замечание ниже.

#### Что сверено (круг 2)

- **Живой проект (MCP)**: `ap_flow_structure` + `includeInput` по всем шести
  флоу — состав шагов и ветки совпадают с журналом; `ap_export_flow` по
  `staff-events-api` (`B3QZyGQCC9RtlYT3noUtg`) и `reg-profile`
  (`HAeSmWAMGEqsf0M1uiVqu`) — `flows[0].id` = `_manifest.json`,
  `state: LOCKED`; `ap_list_flows` — все шесть ENABLED/published.
- **Сентинелы**: живой код `checkin-api/step_14,15`,
  `checkin-counter-api/step_9`, `staff-events-api/step_6`, `reg-profile/step_3`
  (в экспорте) отдаёт `X !== '' ? X : '__none__'`, фильтры читают `…OrNone`;
  `step_2`/`step_4`/`step_6`/`step_13` у шести флоу ссылаются ровно на
  сентинельные значения. `reg-consent-pdn/step_2`, `reg-consent-mkt/step_2` —
  то же.
- **Прогоны прочитаны, а не приняты на слово**: `checkin-api`
  `q0XPjE3xoRmoxf9ifJmBW` (`eventId:"demo"` → `step_14="demo"`,
  `step_15="__none__"`, `step_3/6/13=[]` → `403 forbidden`; `step_8` падает
  `404` штатно, `step_9` отдаёт `403`); `staff-events-api`
  `vqcDOYg6SMnUb5pfrhmB5` (`initData:"garbage"` → `step_6="__none__"` →
  `step_2=[]` → `401 invalid_init_data`).
- **Таблицы**: `events` — `mu9rqipgetmp`/`mu9uzchnu2il`, `event_staff` —
  `mu9rqipgetmp`/`5895728710`, `registrations` — 5 строк без
  `demo-322876545`. Это подтверждает и обоснованность хвоста STF-2, и
  неполноту правки замечания 2.
- **Репозиторий**: `76d25ff` меняет только `AGENTS.md`,
  `catalog/flows/checkin-api.md`, журнал — `flows/` не тронуты
  (`git diff 719d160 76d25ff -- flows/` пуст); `migrations` — 6 строк
  `publish` W102, `version_id` = манифест; `flows/_manifest.json` — те же
  версии, что в журнале. Офлайн-проверки запущены заново:
  `check-export-secrets.sh` — 0 совпадений/0 auth вне ссылок;
  `check-texts.py i18n/ru.json flows/*.json` — 254 пары, 0 расхождений;
  `check-commands.py i18n/*.json flows/*.json` — 0 нарушений;
  `check-agents.py` — 0 нарушений. `check-migrations.py` без ключа —
  не запускается (шаг 0.7), как и в круге 1.
- **Каталог против реальности**: `grep -rn demo catalog/` — кроме
  `catalog/snippets/hmac-qr.md:146` (там `demo` — контрольная пара в sample
  data `fn-verify-qr`, утверждение верное) остаётся один устаревший абзац в
  `catalog/overview.md`.

#### Замечание (круг 2)

1. **важно** — `catalog/overview.md:63-66` утверждает текущим временем:
   «В `events`/`registrations`/`event_staff` намеренно оставлена фикстура
   `demo` — нужна ревьюеру … Подробности — `checkin-api.md`». Живого проекта
   это не описывает (проверено: ни события `demo`, ни staff-записи
   `demo`/`322876545`, ни регистрации `demo-322876545` нет), а правка
   `76d25ff` убрала ту же заметку только из `catalog/flows/checkin-api.md` —
   теперь ссылка «Подробности — `checkin-api.md`» ведёт в никуда. Каталог
   обязан совпадать с проектом (REVIEW-CHECKLIST п. 2), поэтому исправление
   круга 1 осталось половиной. Где: `catalog/overview.md:63-66`. Что сделать:
   убрать абзац (либо переписать под фактическое состояние — фикстуры нет).
   Замечание к каталогу, не к логике W102.
   - *Исправлено*: абзац про фикстуру `demo` убран из `catalog/overview.md`
     (2026-09-24); `grep -rn demo catalog/` больше не находит устаревших
     утверждений (остаётся только легитимная контрольная пара в
     `catalog/snippets/hmac-qr.md`).

#### Итог (круг 2)

Замечание 1 круга 1 принято (конвенция зафиксирована в `AGENTS.md`);
замечание 2 закрыто частично — осталась та же устаревшая заметка в
`catalog/overview.md`. Логика W102 коммитом `76d25ff` не менялась (доки),
живые версии шести флоу и `migrations`/манифест согласованы, офлайн-проверки
зелёные, новых проблем в самом флоу-решении не найдено. Блокеров нет;
пакет — на доработку одного каталожного абзаца, после чего может быть
`готов`.
...
- **Коммит `fce0cc0` — docs-only**: `git show --stat` — `catalog/overview.md`
  (−5 строк, абзац про фикстуру `demo`) и журнал (+90); `flows/` не тронуты.
  `git diff 76d25ff fce0cc0 --stat` — тот же состав. Логика W102 не менялась,
  регрессу неоткуда взяться.
- **Замечание «важно» круга 2 закрыто.** `grep -rn demo catalog/` даёт
  единственное совпадение — `catalog/snippets/hmac-qr.md:146`: контрольная пара
  `(demo, 322876545)` в sample data `fn-verify-qr` (утверждение о контрольном
  значении `aIxmwbnzb_`, а не о живом событии) — легитимно. Устаревших
  утверждений нет. `grep -rn фикстур catalog/` — пусто; `grep -rn
  "намеренно\|ревьюеру\|Подробности" catalog/` не находит ничего про `demo`.
  В `catalog/overview.md` строки 63–66 теперь заголовок «Переменные и
  Connections» — абзац и ссылка «Подробности — `checkin-api.md`» удалены
  целиком, битой ссылки не осталось. `catalog/flows/checkin-api.md` заметки
  про `demo` не содержит (убрана в `76d25ff`).
- **Живой проект (MCP)**: `ap_list_flows` — все шесть флоу W102
  ENABLED/published. `ap_flow_structure` + `includeInput` по всем шести:
  сентинелы на месте (`checkin-api/step_14,15`, `checkin-counter-api/step_9`,
  `staff-events-api/step_6`, `reg-profile/step_3`, `reg-consent-pdn/step_2`,
  `reg-consent-mkt/step_2`), фильтры `step_3/6/13`, `step_3/4`, `step_2`,
  `step_2`, `step_6`, `step_4` читают ровно `…OrNone`; топология линейная,
  посторонних рёбер нет.
- **`ap_read_step_code`** по трём добавленным CODE — `X !== '' ? X :
  '__none__'` (`checkin-api/step_14`, `checkin-api/step_15`,
  `staff-events-api/step_6`).
- **`ap_export_flow` по `checkin-api`** — `flows[0].id ==
  u3Owwfcc9NltCLlLiCfd9` (= `_manifest.json`), `state: LOCKED`.
- **`ap_find_records migrations`** — 6 строк `publish` W102, `version_id`
  совпадают с манифестом и журналом.
- **Офлайн-проверки запущены заново**: `check-export-secrets.sh` — 0
  совпадений, 8 `BOT_TOKEN`/2 `QR_SIGNING_KEY` как ожидалось, 0 `auth` вне
  ссылок; `check-texts.py` — 254 пары, 0 расхождений; `check-commands.py` — 0;
  `check-agents.py` — 0. `check-migrations.py` без ключа платформы в среде не
  запускается (шаг 0.7 ROADMAP) — как и в кругах 1–2; состав манифеста (29)
  против `ap_list_flows` (32) — унаследованный хвост `ChatBot` + два `zz-*`
  (W72/W70/W58), не W102.
- **`AGENTS.md`** — резерв `__none__`/`-` за реальными ключами на месте
  (строки 169–173).

#### Итог (круг 3)

Замечание «важно» круга 2 закрыто полностью: устаревший абзац про фикстуру
`demo` убран из `catalog/overview.md`, других устаревших утверждений в
`catalog/` не осталось (единственное вхождение `demo` — легитимная
контрольная пара в `hmac-qr.md`). Коммит `fce0cc0` docs-only, живой проект и
репозиторий согласованы, офлайн-проверки зелёные. Новых замечаний нет;
прежние «на будущее» (сентинел `__none__` как принятая конвенция, позитивный
различающий прогон STF-2, унификация сентинела) — приняты и вынесены в
хвосты. **Вердикт: замечаний нет** — пакет может быть `готов`.

...
- **#1 (`важно`) — снято.** Живые прогоны `manage-api` (PRODUCTION) найдены и
  прочитаны: `0mhXHZ91D8HJz4yx8ZkBt` (14:40:52) — `participants`
  `mu9rqipgetmp`, `200`, `counters {registered:5, checked_in:1, cancelled:0,
  all_consent:5}`; `U6TxZkiFl7R2JP72r62qg` (14:40:54) — `participants`
  `mu9uzchnu2il` (0 регистраций), `all_consent:5` (глобальный);
  `FsigRJNPL7pwPgSluwPbR` (14:40:59) — не-staff `532804490`, `403 forbidden`.
  Различающая пара на одном входе (то же действие и событие) есть:
  staff `322876545` → `200` с данными, не-staff → `403`. Счётчик сверен
  независимо: `users` с `consent_marketing = true` — ровно 5 строк, все
  `blocked_bot = false` (`ap_find_records`), сходится с `all_consent:5`.
  Временный флоу `zz-w68-mint-init-data` (`bZjnALq3ri4GPQj8tg693`) в
  `ap_list_flows` отсутствует; `migrations 2026-09-23-w68-04` (action
  `delete`) на месте.
- **#7 — снято.** Шапка `catalog/flows/menu.md` перечисляет входы W73
  (обычный текст) и W68 (`menu:*`); структура флоу это подтверждает.
- **#3 — снято.** Гоча 19 в `AGENTS.md` согласуется с наблюдениями:
  `ap_resolve_property_options` для `callFlow/flow` отдаёт `menu` с
  `callbackData`/`callbackQueryId`, а `ap_export_flow` (LOCKED) — без них;
  рантайм несёт значения через `flowProps.payload` (подтверждено живым
  прогоном). Формулировка «и в черновике, и в LOCKED» не проверена на
  черновике, но фактам не противоречит.
- **#2, #4, #5, #6 — принятие обоснованно**, не отговорка: #2 — видимость,
  не граница прав, а реальная граница доказана M3; #4 — ретрофит-ROUTER на
  шаг с существующим потомком в этом движке действительно не гейтит
  (гоча 10), цена `continueOnFailure` названа; #5 — тот же приём, что у меню
  на `/start`; #6 — реализация верна серверному сегменту, и расхождение
  теперь названо в «Ответе владельца».

Новых замечаний нет. Диф `e0aed17..e1cd66b` трогает только `AGENTS.md`,
`catalog/flows/menu.md` и журнал; `flows/*.json` и `_manifest.json` не
менялись, поэтому сверка версий круга 1 остаётся в силе. Офлайн-проверки
перезапущены и зелёные (`check-export-secrets.sh` 0; `check-texts.py` 254/0;
`check-commands.py` 0; `check-agents.py` 0; `prototypes/check.mjs` OK).
`check-migrations.py` по-прежнему недоступен без ключа платформы —
ограничение, не замечание; три флоу пакета сверены через MCP.
...
- **2026-09-23** — ревью круг 2: «замечаний нет»; все замечания круга 1
  закрыты. Пакет — `готов`, PR #159 смержен в `main`.

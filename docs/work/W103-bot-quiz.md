...
- **Контент обновлён 2026-09-24**: 8 авторских вопросов на трёх языках
  (UZ/EN/RU) в `quiz_questions` викторины `booth1`; порядок 1–8 сохранён
  (вопросы 4 и 8 ссылаются на предыдущие). **Окно остаётся плейсхолдером**
  `2026-09-23…2026-10-01` — владелец выставляет реальные даты перед событием.

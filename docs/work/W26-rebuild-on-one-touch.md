...
- [ ] STF-2 различающим прогоном: не-контролёр, контролёр чужого ивента,
      отозванный — `403`; настоящий — `200`
- [ ] IDM-2: повторный чекин показывает исходное время
- [ ] IDM-4: один `update_id` дважды → одна реакция
- [ ] PAR-1/PAR-2: `consent_marketing` не проставляется побочным эффектом
- [ ] переход между касаниями поднимается из `sessions`, а не из памяти прогона
- [ ] доказательства сделаны после `ap_lock_and_publish`, вывод скопирован сюда
- [ ] `catalog/` совпадает с живым проектом
- [ ] в `overview.md` отвечено, чем лечится расхождение каталога с проектом

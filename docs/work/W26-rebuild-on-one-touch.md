...
- [x] независимое ревью, вердикт «замечаний нет» (три круга: 3 важно + 3 на
      будущее в первом, все подтверждены исправленными во втором + 1 новое
      важно + 1 на будущее, всё закрыто в третьем — «замечаний нет»)
...
- **Ревьюер**: независимый агент (свежий контекст) · **Дата**: 2026-09-13 ·
  **Вердикт**: есть замечания

Задача этого прохода — проверить, что шесть пунктов первого круга исправлены
по-настоящему (не только заявлены), и заново пройтись по чек-листу с упором на
`checkin-api`/`my-qr-api` (пересобраны физически, максимальный риск регрессии).

### Проверка исправлений первого круга

1. **Подтверждено, исправлено полностью.** `ap_flow_structure` на обоих флоу
   показывает `trigger → step_1(verify initData) → ROUTER(step_10/step_2)`,
   и всё дальнейшее (`tables-find-records`, три `callFlow` к `fn-*`, чтение
   имени) лежит строго внутри ветки `valid`; ветка `Otherwise` — два шага
   (CODE + `return_response`), без единого обращения к таблицам или `callFlow`.
   Прогнано `ap_test_flow` с мусорным `initData` на обоих флоу (`zVh9mDIuDOi0pAs3xy13N`
   checkin-api, `7ufrDkCRIf3VJ2G3Jkidj` my-qr-api): в списке выполненных шагов
   только `trigger`/`step_1`/router/error-ветка — ни `tables-find-records`,
   ни один `callFlow` не запустились. Позитивный путь не сломан: прочитан
   прогон `XZGzjgTk3PoN74Vkyrsk5` (checkin-api, реальный `initData`
   `@binali_beelab`, `eventId: demo`) — полная цепочка `step_2…step_9`
   отработала, результат `already`/`200` с корректным временем по Ташкенту, и
   `VPPFe0ZbkdVqimmvQRcKZ` (my-qr-api, тот же контролёр) — `not_registered`/`200`
   по цепочке `step_3…step_6`. Читаемый REST-экспорт (`GET /flows/.../template`)
   подтверждает то же самое на уровне конфигурации PIECE-шагов: ROUTER сразу
   после `step_1`, `only_if: checked_in_at not_exists` на `step_8`, фильтр
   `event_staff` по `(event_id, telegram_id, revoked_at not_exists)` не изменился.
   Одна деталь найдена, но не меняет вердикт по этому пункту (см. новую находку
   ниже про каталог).
2. **Подтверждено, исправлено.** По всем 12 флоу (`ap_read_step_code` на каждый
   текстонесущий CODE-шаг) строки читаются из `inputs.texts[key]` с простой
   подстановкой `{var}`, литералов не найдено. Каждое значение сверено с
   `i18n/ru.json` построчно — расхождений нет, включая контрольные точки из
   задания: `wizard.hint.datetime`/`wizard.err.datetime` описывают формат
   `ДД.ММ.ГГГГ ЧЧ:ММ` (не ISO), `wizard.ask.geo`/`wizard.err.geo_expected`
   упоминают пропуск через `-`, `wizard.publish.change_summary` использует
   `{title}`/`{changes}`, `field.geo` — используется в `event-wizard-publish/step_7`
   для diff гео единым полем вместо `lat`/`lon` по отдельности. Отдельно
   проверены прежде «нулевые» шаги (`reg-start/step_13`, `reg-consent-pdn/step_13`,
   `event-wizard-publish/step_5`) — это настоящие no-op (`{ignored: true}`),
   литералов там нет и не было.
3. **Подтверждено, исправлено.** `hmac-init-data.md`, `parse-start.md`,
   `find-registration.md` больше не ссылаются на удалённые флоу/шаги, все три
   говорят «встраивает только канонический `fn-*`». `hmac-qr.md` описывает
   ровно `fn-sign-qr`/`fn-verify-qr`. Код в файлах сверен побайтово с живыми
   `ap_read_step_code` по всем пяти `fn-*` (`fn-hmac-init-data/step_1..3`,
   `fn-sign-qr/step_1..2`, `fn-verify-qr/step_1..2`) — совпадает дословно,
   вплоть до комментариев. `fmt-time.md`/`event-card.md` помечены устаревшими
   с объяснением (ADR-0014, инлайн-форматирование) и таблицей реальных копий
   вместо списка мёртвых флоу. `ru-texts.md` и `README.md` описывают W26-вариант
   точно так, как он построен.
4. **Подтверждено, исправлено.** `catalog/tables/events.md` больше не
   приписывает визарду запись `overbook_pct`; `event-wizard-publish/step_7`
   (REST-экспорт, вход `step_8`) действительно не содержит поля `overbook_pct`
   среди записываемых значений — заметка соответствует реальности.
5. **Подтверждено, исправлено.** Таблица «Что построено» включает все 18
   флоу (12 из исходного счёта + 6 визарда), раздел «Как проверено» заполнен
   сводкой со ссылками на конкретные шаги/прогоны.
6. **Подтверждено, состояние не изменилось (осознанно отложено).**
   REST-экспорт `tg-router` показывает: `step_12`–`step_15`
   (`reg-start`/`reg-consent-pdn`/`reg-consent-mkt`/`reg-phone`) действительно
   несут в `flowProps` одновременно плоские поля верхнего уровня и обёртку
   `payload` с теми же данными; `step_17`–`step_22` (все шесть вызовов визарда)
   содержат только `payload`, без мусора. Функционально безвредно — callee
   читает исключительно `payload` (подтверждено самими прогонами: маршруты
   регистрации работают). Оценка владельца точна, менять не требуется.

### Новая находка (не из первого круга)

7. **важно** — Пересборка `checkin-api`/`my-qr-api` (исправление замечания 1)
   изменила топологию обоих флоу физически, но `catalog/flows/checkin-api.md`
   и `catalog/flows/my-qr-api.md` **не обновлены** и описывают старую,
   уже отвергнутую схему. Дословно: `checkin-api.md` — «Флоу **линейный, без
   ROUTER**: все проверки читаются заранее... Это сознательный выбор», и
   таблица шагов перечисляет только `trigger, step_1…step_9` по прямой цепочке.
   Реальная структура (см. `ap_flow_structure`, снятая в этом ревью) —
   `trigger → step_1 → step_10 (ROUTER: valid/Otherwise)`, с `step_2…step_9`
   внутри ветки `valid` и `step_11/step_12` внутри `Otherwise` — 13 шагов,
   не 9, и ROUTER есть. `my-qr-api.md` содержит ту же ошибку: «Линейный, без
   ROUTER (тот же принцип...)», таблица `trigger, step_1…step_5`, тогда как
   реально `step_2` — ROUTER, `step_3…step_6` внутри `valid`, `step_7/step_8`
   внутри `Otherwise` — 9 шагов, не 6. Это прямое нарушение чек-листа (п. 2:
   «шаги в описании соответствуют `ap_flow_structure` — не «примерно», а по
   списку») и правила синхронизации каталога из CLAUDE.md («После **любого**
   изменения flow через MCP: 1. `ap_flow_structure`... 2. обновить
   `catalog/flows/<flow>.md`»). Особенно неприятно то, что карточка не просто
   устарела, а **утверждает нечто прямо противоположное** тому, что было
   сознательно исправлено этим же пакетом по замечанию 1 — «линейный без
   ROUTER» был как раз тем дизайном, который замечание 1 признало дырой в
   устойчивости и потребовало заменить на ROUTER. Функционально это не бага
   (флоу работают правильно, см. проверку замечания 1 выше), это чисто
   расхождение каталога с реальностью — но именно такое расхождение,
   описывающее несуществующую топологию, каталог, по формулировке
   `snippets/README.md`, называет «хуже пустого: по нему принимают решения».
   Нужно переписать таблицы шагов обеих карточек по актуальной структуре
   (включая ROUTER и обе ветки) и убрать формулировку «без ROUTER».

### На будущее (не блокирует)

8. **на будущее** — Ни одна из девяти карточек `catalog/flows/*.md` для
   остальных текстонесущих флоу (`reg-consent-pdn`, `reg-consent-mkt`,
   `reg-phone`, `event-wizard-start`, `event-wizard-edit-start`,
   `event-wizard-field`, `event-wizard-photo`, `event-wizard-geo`,
   `event-wizard-publish`) не упоминает конвенцию `inputs.texts`/ADR-0014,
   применённую по замечанию 2 (`reg-start.md` упоминает ADR-0014 мимоходом,
   но тоже не описывает механизм `texts`). Сами карточки при этом
   структурно точны (шаги/ROUTER-ветки описаны верно — проверено по
   `event-wizard-field.md`/`reg-start.md` построчно против `ap_flow_structure`),
   расхождения с реальностью нет, только неполнота. Стоит дополнить разделом
   «тексты — через `inputs.texts`, см. `ru-texts.md`» при следующей правке
   этих карточек.

### Итог

Все шесть замечаний первого круга закрыты по-настоящему: пересборка
`checkin-api`/`my-qr-api` проверена и на конфигурации (REST), и на поведении
(`ap_test_flow` на мусорном и на реальном `initData`), тексты и снипеты
сверены построчно/побайтово, `events.md` и раздел «Как проверено» приведены
в порядок, п. 6 корректно оставлен как задокументированный хвост. Новая
находка (п. 7) — не регрессия поведения, а рассинхронизация каталога с
реальностью на самом рискованном месте пакета; она достаточно весома
(нарушает и чек-лист, и собственное правило каталога), чтобы не ставить
«замечаний нет» до её исправления. Пункт 8 — не блокер.

Пакет уходит на **повторное** исправление (только п. 7, п. 8 — по желанию),
затем — снова на ревью.

### Ответ владельца пакета (по итогам повторного ревью)

7. **Исправлено.** `catalog/flows/checkin-api.md` и `catalog/flows/my-qr-api.md`
   переписаны по актуальной структуре (снята повторно `ap_flow_structure` после
   правки): таблица шагов отражает `ROUTER` (`step_10`/`step_2`) с обеими
   ветками, столбец «Где решается» в таблице HTTP-статусов `checkin-api`
   указывает, что `invalid_init_data` теперь решает `ROUTER`, а не `step_7`.
   Формулировка «линейный, без ROUTER» убрана из обеих карточек. Добавлена
   ссылка на гочу #10 (почему ROUTER собирался через удаление и пересборку
   цепочки, а не прямую вставку).
8. **Исправлено, раз уж дёшево.** Все девять оставшихся карточек
   (`reg-consent-pdn`, `reg-consent-mkt`, `reg-phone`, `event-wizard-*` ×6)
   дополнены строкой про `inputs.texts`/ADR-0014 со ссылкой на `ru-texts.md`.
...
Задача этого прохода — точечно проверить два пункта повторного ревью
(находка №7 — карточки `checkin-api`/`my-qr-api` не отражали пересборку с
`ROUTER`; №8 — девять карточек не упоминали `inputs.texts`/ADR-0014), не
переделывая аудит с нуля.

### Проверка п. 7

`ap_flow_structure` снята заново на оба флоу и сверена построчно с таблицами
шагов в карточках:

- **`checkin-api`** (`rKoDYtiIVdbzlW59b57uH`) — живая структура: `trigger →
  step_1 (PIECE) → step_10 (ROUTER) → {valid: step_2…step_9 (PIECE×6, CODE
  step_7, PIECE step_8/step_9), Otherwise: step_11 (CODE) → step_12 (PIECE)}`,
  13 шагов. Таблица в `catalog/flows/checkin-api.md` перечисляет ровно эти
  13 имён с теми же типами (PIECE/ROUTER/CODE) и той же принадлежностью ветке
  (`valid`/`Otherwise`) — совпадение не «примерно», а по списку. Фраза
  «линейный, без ROUTER» из карточки убрана; единственное упоминание
  «линейным без ROUTER» осталось в тексте намеренно, в прошедшем времени
  («До этого флоу был линейным без ROUTER; переход понадобился...») — это
  описание истории изменения, а не текущего состояния, противоречия нет.
- **`my-qr-api`** (`WYmnxVM4xPAWZA1IvNZok`) — живая структура: `trigger →
  step_1 (PIECE) → step_2 (ROUTER) → {valid: step_3/step_4 (PIECE) → step_5
  (CODE) → step_6 (PIECE), Otherwise: step_7 (CODE) → step_8 (PIECE)}`,
  9 шагов. Таблица в `catalog/flows/my-qr-api.md` совпадает построчно:
  те же 9 имён, те же типы, та же ветка. Формулировка «линейный, без ROUTER»
  тоже убрана.

Дополнительно прочитан код `checkin-api/step_11` и `my-qr-api/step_7` —
оба формируют ответ `401` через `inputs.texts['checkin.unauthorized']`
(строка совпадает с `i18n/ru.json`), без литерала — карточки не только верны
по топологии, но и по заявленному использованию `inputs.texts` в ветке
`Otherwise`. **П. 7 подтверждён как исправленный.**

### Проверка п. 8

Проверены все девять оставшихся карточек: `reg-consent-pdn`, `reg-consent-mkt`,
`reg-phone`, `event-wizard-start`, `event-wizard-edit-start`,
`event-wizard-field`, `event-wizard-photo`, `event-wizard-geo`,
`event-wizard-publish`. В каждой в разделе «Заметки» есть строка
«Тексты — через `inputs.texts`, не литералом в коде (ADR-0014); значения
сверены с `i18n/ru.json`, механизм — `ru-texts.md`» (grep по всем девяти
файлам подтверждает присутствие, без исключений). **П. 8 подтверждён как
исправленный.**

### Дополнительно

Пока читались карточки, явных новых расхождений с `ap_flow_structure`,
битых ссылок или противоречий не найдено. Новых замечаний не заводится.

### Итог

Оба пункта повторного ревью (7 — важно, 8 — на будущее) закрыты по-настоящему,
проверено по живой структуре флоу, а не только на словах владельца. Все три
круга ревью пройдены, все замечания закрыты. Пакет W26 готов к статусу
«готов» — остаётся формально проставить статус и снять чек-бокс
«независимое ревью» в чек-листе готовности.
...
- **STF-2** — закрыт всеми четырьмя сценариями на реальной подписи, см. журнал.
  Больше не блокер.
- **Независимое ревью пройдено, три круга, вердикт «замечаний нет».** Пакет
  покрыл и исходную область W26 (регистрация, чекин), и слитый W11 (визард
  ивента) одним ревью. Статус — «готов».
...
## Хотфикс после закрытия (2026-09-13, вне цикла ревью пакета)

Найдено при живом сквозном тесте владельцем (создание ивента через
`/newevent` → диплинк для регистрации), уже после статуса «готов»,
отдельным агентом — **не правка чужого журнала задним числом**, а фиксация
находки в момент, когда её сделали:

1. **OWN-6 (ссылка регистрации) не была реализована.** `event-wizard-publish`
   слал владельцу фиксированный текст `"Ивент опубликован!"` без диплинка;
   `BOT_USERNAME` не читался ни одним флоу (см. `catalog/variables.md` до
   этой правки). Ключ `wizard.published` с плейсхолдером `{url}` уже лежал
   в `i18n/ru.json` (и `uz`/`en`), но был сиротой — ни один флоу его не
   использовал. Исправлено: `step_7` теперь строит `registrationUrl` и шлёт
   его владельцу через `wizard.published` при создании (`!isEdit`).
2. **Найден и исправлен блокер серьёзнее — id создаваемых ивентов не проходил
   `fn-parse-start`.** Генератор `id` добавлял свой собственный префикс `e`
   (`id = 'e' + Date.now().toString(36) + random(4)`, 13 символов). Диплинк
   собирается как `e<id>`, то есть у любого ивента, созданного визардом,
   payload получался `e` + `e...` = 13-символьный `eventId` после разбора —
   `SLUG` в `fn-parse-start` (`^[A-Za-z0-9_]{1,12}$`) такой не пропускает.
   Итог: **ни одна ссылка регистрации ни на один визард-ивент не могла
   сработать** — `tg-router` → `fn-parse-start` стабильно отдавал
   `bad_event_id`, различающим прогоном `bDoeR2ExYoiiBfrWXjd5B` (payload
   `eemu022plsnokv`, `error:"bad_event_id"`) подтверждено, что причина
   именно в разборе, а не в доставке `/start`. Тот же `SLUG` использует
   `fn-sign-qr`/`fn-verify-qr`, так что QR на такой ивент тоже не сошёлся бы.
   Исправлено — генератор больше не добавляет `e` в сам `id`
   (`Date.now().toString(36) + random(4)`, 12 символов). Опубликовано,
   `ap_validate_flow` — чисто.
3. **Существующий тестовый ивент владельца** (создан до фикса, `id`
   `emu022plsnokv`) переведён на корректный формат прямым `ap_update_record`
   (`id` → `mu022plsnokv`) — по разрешению владельца, регистраций на нём ещё
   не было, миграции данных не требовалось.

Каталог (`catalog/flows/event-wizard-publish.md`) обновлён под текущий код.
Независимого ревью этот хотфикс не проходил — пакет W26 уже закрыт, и это
не переоткрывает его статус «готов»; если правка сочтётся достаточно
значимой, ей место в отдельном пакете с полным циклом сдачи.

4. **Второй раунд того же теста нашёл ещё две проблемы в `event-wizard-field`
   (`step_1`)**, обе — при вводе владельцем даты `11.11.2016` вместо `2026`:
   - **`reg-start/step_5` не передавал переводы отказных причин** (`reg.deadline_passed`,
     `reg.no_seats`, `reg.event_cancelled`, `reg.event_finished`, `reg.not_published`,
     `event.card.not_found`) — во входе `texts` был только `common.err.generic`,
     поэтому участнику уходил сырой ключ i18n вместо текста (нарушение
     инварианта CLAUDE.md п. 7). Исправлено — добавлены все шесть ключей,
     значения byte-в-byte из `i18n/ru.json`.
   - **Визард не проверял, что `starts_at`/`reg_deadline_at` — дата в будущем.**
     Прошедшая дата принималась без вопросов, ивент публиковался, и участник
     получал `reg.deadline_passed` при первой же попытке зарегистрироваться —
     ивент оказывался неработоспособным сразу после создания, без единого
     предупреждения владельцу. Добавлена проверка `d.getTime() <= Date.now()`
     на обоих полях (новые ключи `wizard.err.starts_at_past`/
     `wizard.err.deadline_past`, добавлены в `i18n/{ru,uz,en}.json`).
     Проверено `ap_test_step` на опубликованной версии: прошедшая дата
     (`11.11.2016 10:00`) → `outcome:"error"`, текст на русском, `nextStep`
     пуст (`vMaNZUwGhHAe1bi5b3f74`); позитивный контроль — будущая дата
     (`20.12.2026 10:00`) → `outcome:"next"`, `draft.starts_at` записан
     (`z0Hy8Oj9yH1PQf35ILUCy`).
   Оба флоу (`reg-start`, `event-wizard-field`) опубликованы,
   `ap_validate_flow` — чисто на обоих. Карточки
   [`event-wizard-field.md`](../../catalog/flows/event-wizard-field.md) обновлены.

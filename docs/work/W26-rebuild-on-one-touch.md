...
- [ ] правка опубликованного шлёт уведомление зарегистрированным **только** при
      изменении полей из [notify-on-change](../DATA-MODEL.md#notify-on-change),
      с перечнем «было → стало»
- [ ] состояние визарда переживает рестарт воркера (лежит в `sessions`)
- [ ] авторизация правки — только `events.owner_id == telegram_id` инициатора
- [ ] `event-wizard-field` (ADR-0016) проверен на все шесть полей, включая

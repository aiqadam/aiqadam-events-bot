...
- **Статус**: заблокирован — граф dev пересобран, опубликован и проверен;
  осталось переснять `flows/*.json` + `_manifest.json` (нужен ключ платформы,
  шаг 0.7 — человек) и пройти независимое ревью
- **Владелец**: агент
...
**Блокер (почему не `готов`):**

- `flows/*.json` + `_manifest.json` **не пересняты**: на машине нет ключа платформы
  (`QADAM_API_KEY` пуст, шаг 0.7 — человек), а MCP-путь (`ap_export_flow` +
  `tools/export-flow-mcp.py`) для 31 большого флоу неподъёмен по объёму
  (≈1,1 МБ inline). Экспорт в репозитории сейчас описывает dev **до** пересборки
  (`callFlow` на старые externalId). Лечение — `tools/export-flows.sh` с ключом
  (пересоберёт `flows/*.json` и `_manifest.json` за один прогон) либо тот же
  экспорт через MCP по одному флоу.
- Независимое ревью не запускалось (пакет не доведён).

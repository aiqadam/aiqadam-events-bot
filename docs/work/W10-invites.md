...
### Повторное ревью (круг 3, 2026-09-21)

- **Ревьюер**: opencode (deepseek-v4.1-flash, независимый агент, чистый контекст) · **Дата**: 2026-09-21 · **Вердикт**: блокеров и «важно» нет; одно замечание уровня «на будущее» — пакет готов к `готов`.

**Закрытие замечания круга 2** (проверено живыми артефактами, не по тексту ответа):

- **Q60 внесён в указатель — закрыто.** Merge-коммит `26dac61` (PR #104) трогает только `docs/OPEN-QUESTIONS.md` и `docs/work/W10-invites.md` (`git show --stat`); строка `| [Q60](#q60) | … |` стоит в таблице-указателе (строка 69), секция `## Q60. …` — в конце файла (строка 3439). Нумерация не сбита: секций `## Qn` ровно 60. `flows/`, `i18n/` и код Mini App в коммите не менялись, поэтому структурный регресс флоу исключён.

**Новое замечание:**

1. **на будущее** Ссылка `[Q60](#q60)` в указателе — **мёртвая**: у секции Q60 нет якоря `<a id="q60"></a>`, который есть у всех Q1–Q59. В отрендеренном GitHub (проверено на `github.com/.../docs/OPEN-QUESTIONS.md`) элемент `id="user-content-q60"` отсутствует, тогда как у Q58/Q59 он есть; для секции существует только автоякорь заголовка `id="user-content-q60-токен-инвайта-staff-…"`, а `href="#q60"` в строке указателя — есть, то есть цель ссылки не резолвится. Туда же ведут ссылки из `docs/SECURITY.md` и из ответа владельца в этом журнале — они тоже не переходят. Не блокирует: Q60 в указателе виден и находится поиском по файлу; чинится добавлением одной строки `<a id="q60"></a>` перед заголовком секции. Где: `docs/OPEN-QUESTIONS.md`, перед строкой 3439. Это отдельный дефект от замечания круга 2 (строка в указателе была добавлена, а якорь секции отсутствовал с круга 1).

**Регресс:** нет. `flows/`, `i18n/`, `miniapp/`, таблицы и переменные с момента круга 2 не менялись (merge PR #104 docs-only); живой проект подтверждает прежнее состояние.

**Проверки круга 3 (свои запуски):**

- **Живой проект (MCP):** `ap_validate_flow` — `staff-invite` 16/16, `staff-accept` 13/13, `tg-router` 22/22, все «ready to publish»; `ap_export_flow` — версии `cYLX8SyKdGhxp3Bu6PHE1` / `8ath7lNU6cmkgNUS2fZh7` / `pCZzbj5UKQAxztYtD4Oba`, все `state: LOCKED` / `status: PUBLISHED`, побайтово совпадают с `flows/_manifest.json` и строками `migrations` `2026-09-21-w10-01..04`.
- **Данные:** `staff_invites` — 0 записей, `event_staff` — 0 (замечание 2 круга 1 остаётся закрытым, мусорной строки нет).
- **Прогоны перечитаны `ap_get_run`:** `FT1Fe6UliSl65Wgm6e8GI` — `Environment: TESTING`, ветка `ok` (`used_at`/`used_by` + строка `event_staff` + кнопка сканера); `vpwLPv92CrRjabDnhn3yi` — `Environment: PRODUCTION`, ветка `invalid`. Журнал и «Хвосты» этому соответствуют (замечание 1 круга 1 закрыто).
- **Офлайн-гейты (запущены сам, с аргументами pre-commit):** `check-texts.py i18n/ru.json flows/*.json` → 27 флоу / 230 пар / 0, exit 0; `check-commands.py i18n/*.json flows/*.json` → 27 флоу / 0 нарушений, exit 0; `check-export-secrets.sh` → чисто (`BOT_TOKEN` 7, `QR_SIGNING_KEY` 2, все поля `auth` — ссылками), exit 0.
- **Дерево:** `git status` чист, ветка `main` синхронна `origin/main`; `git log --oneline -6` — вершина `26dac61` (merge PR #104).
- **Каталог ↔ живой проект:** `catalog/flows/staff-invite.md`, `staff-accept.md`, `tg-router.md` (в т. ч. ветка `staff_accept` в `step_10` и `step_21 callFlow staff-accept`), `catalog/overview.md` (27 флоу) — совпадают со структурой и экспортом.

**Ограничения (названы честно):**

- Живых позитивов создания `staff-invite` (кнопка Mini App с настоящим `initData`) и успешного приёма в PRODUCTION нет — хвост на владельца/приёмку [W15](../BACKLOG.md#w15-приёмка); негативы (`401`, все отказы приёма) доказаны.
- Гонка claim'а (`staff-accept/step_7`, ветка `onFailure`) живым прогоном не проверялась — механизм подтверждён структурой и документацией платформы, отдельный диагностический флоу — хвост.
- `tools/check-migrations.py` без `QADAM_API_KEY` не запускается (exit 2, ключа и Keychain на машине нет); сверка манифест ↔ живой `ap_export_flow` ↔ `migrations` сделана вручную по MCP и сошлась.

### Ответ владельца пакета (круг 3, 2026-09-21)

1. **Исправлено:** добавлен `<a id="q60"></a>` перед секцией Q60 — ссылки
   `[Q60](#q60)` из указателя, `SECURITY.md` и журнала резолвятся.

**Закрытие пакета.** Единственное замечание круга 3 — неблокирующее
«на будущее» и исправлено; блокеров и «важно» нет на всех трёх кругах.
Живые позитивы (создание из Mini App и успешный приём в PRODUCTION) и гонка
claim'а оставлены хвостами на владельца/приёмку — так же закрыт W55.
Пакет — `готов`.

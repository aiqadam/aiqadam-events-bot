# W104. Две среды в одном репозитории; prod-хотфикс после копии

- **Статус**: готов (решением владельца 2026-09-26)
- **Владелец**: агент
...
> Пересборка `events-dev` (вариант «а») **вынесена в [W106](W106-dev-rebuild.md)** —
> отдельный пакет под другого агента. Здесь остаётся модель сред и prod-хотфикс.

## Цель

Ввести среду как first-class объект репозитория ([ADR-0042](../adr/0042-two-environments-one-repo.md))
и починить prod после переноса dev→prod копией. Пересборка `events-dev` —
в [W106](W106-dev-rebuild.md).
...
- [~] dev: очистка + пересборка графа (вариант «а») — **вынесено в [W106](W106-dev-rebuild.md)**
- [~] `catalog/environments.md` / `catalog/` по dev — **в W106** (id dev обновятся при пересборке)
- [x] независимое ревью — 2 круга (круг 2: каталог закрыт; независимая проверка prod недостижима из-за отсутствия prod-OAuth у субагента — **prod принят владельцем** 2026-09-26)
...
- **2026-09-26** — **закрытие W104 решением владельца**: prod принят (независимая проверка prod недостижима — у субагента-ревьюера нет OAuth prod-MCP; прод проверен владельцем); замечания каталога круга 1 закрыты, счётчики `overview.md` — 32/17; пересборка dev вынесена в [W106](W106-dev-rebuild.md).

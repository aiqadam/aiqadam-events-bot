...
- [ ] чтения `users` с ПД сужены через `columns`: **1 из 6** —
  `tg-router/step_7` сделан и проверен сквозным прогоном; остались
  `checkin-api/step_28`, `checkin-api/step_12`, `my-qr-api/step_15`,
  `fn-resolve-segment/step_4`, `fn-resolve-segment/step_7`

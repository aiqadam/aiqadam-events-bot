...
факты были заново собраны в живой временной паре `tmp_delete_demo` +
`zz-w98-delete-demo`, которая **держалась до вердикта повторного ревью**
(приём из гочи 17/17, как `zz-diag-skip-primitive`), а после вердикта
«замечаний нет» **удалена**; в `migrations` — строки
`2026-09-23-w98-05/06` (create) и `2026-09-23-w98-07/08` (delete).
Прогон `8fQ3S9LiPT3kvxoYmUqXB` (`ap_test_flow`) читается через `ap_get_run`
(проверено ревьюером круга 2):
...
2. **важно** — доказывающий временный флоу удалён до вердикта ревью, поэтому
   ...
   - *Исправлено*: собрана и **держалась до вердикта повторного ревью** живая
     ...
     `continueOnFailure` (см. «Как проверено»). После вердикта круга 2
     «замечаний нет» пара удалена, в `migrations` — строки delete
     `2026-09-23-w98-07/08`.
...
1. **важно 1** — закрыто. `gh issue view 150 --comments`: Result-комментарий
   опубликован по шаблону (`Delete actions` / `Empty records_ids` /
   `Nonexistent id` / `Deletion` / `By filter` / `Limit per call / time` /
   `Conclusion`).
2. **важно 2** — закрыто. Читаемое доказательство живое и читается:
   - `ap_list_tables` — `tmp_delete_demo` (`VMx3hKSGBYil0EndRkEQm`, 2 записи
     `d2`/`d3`); `ap_list_flows` — `zz-w98-delete-demo`
     (`zakb850cABRbh7NrizHG7`, `draft`/`DISABLED`);
   - `ap_get_run 8fQ3S9LiPT3kvxoYmUqXB` — `SUCCEEDED`: `step_1`
     `tables-delete-record` реального id → `{"success":true}` (hard-delete);
     `step_2` несуществующий id с `continueOnFailure` → ❌ `404
     ENTITY_NOT_FOUND`, прогон продолжен; `step_3` `tables-find-records` →
     остались только `d2`/`d3` (`d1` исчез). Структура флоу — три
     `tables-`шага;
   - `migrations`: строки `2026-09-23-w98-05/06` (`create` для
     `table:tmp_delete_demo` и `flow:zz-w98-delete-demo`, commit `8d11051`).
   - Журнал честно пишет, что пара держится **до вердикта** и затем удаляется.
3. **на будущее 3** — закрыто: в гоче 18 `AGENTS.md` есть явная кросс-ссылка
   на гочи 1/12 (поле шага `table_id` — `externalId`; внутренний id — только
   в `records_ids`).

**Проверено:** MCP (`ap_list_tables`, `ap_list_flows`, `ap_get_run`,
`ap_find_records` по `migrations`), дифф `8d11051`, офлайн-проверки — exit 0.
Временные артефакты не попали в `flows/`, `flows/_manifest.json`, `catalog/`:
grep по репозиторию находит `tmp_delete_demo`/`zz-w98-delete-demo` только в
журнале W98; манифест — 29 опубликованных флоу, совпадает с `ap_list_flows` за
вычетом трёх `zz-*` (draft). Живые флоу/таблицы не менялись (дифф ветки
`p0-checks` к `main` — только `AGENTS.md`, `docs/*`).

**Действие после вердикта (исполнение плана, не замечание):** удалить
`tmp_delete_demo` + `zz-w98-delete-demo` и добавить в `migrations` строки
`delete` (сейчас в таблице только `w98-05/06` `create`).

**Не проверено:** `tools/check-migrations.py` — ключа платформы нет
(`QADAM_API_KEY` пуст, в keychain нет), exit 2; `publishedVersionId` манифеста
сверен только по составу флоу, не по версиям (нужен ключ).

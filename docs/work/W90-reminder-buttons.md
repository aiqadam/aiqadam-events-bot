...
- **2026-09-24 — живая проверка (агентская половина); принято владельцем.**
  Живой прогон крона `reminders` (03:30/03:45 UTC) на инстансе: 24h-напоминание
  ушло владельцу (`322876545`, `message_id 2611`) с кнопкой «Открыть билет»
  (`web_app` → `#/ticket?event_id=…`), 2h — следующим id; событие с непустыми
  `lat/lon` дало **вторую** кнопку «Как добраться»
  (`https://yandex.uz/maps/?ll=69.335264%2C41.341407&z=17&pt=…`, `message_id
  2613`). На повторном тике те же таргеты дали `stored:false` → `already_sent`
  — второго сообщения нет (идемпотентность). Тестовые события/регистрации и
  дедуп-ключи удалены. Физический тап/отрисовка на устройстве — не сделаны
  агентом; см. #117 «Live verification».
...
- Пункт 2 (ссылка на трансляцию вместо адреса для онлайн-события) — только
  после #119 (Phase 3).
- **Живая проверка — сделана и принята владельцем (2026-09-24).** Отправка
  напоминаний (24h/2h + «Как добраться») подтверждена живым прогоном крона,
  см. «Журнал». Агенту недоступен только физический тап/отрисовка на
  устройстве.

...
### Цена шага: что сколько стоит (замер 2026-09-13, после патча #417)

| Тип шага | Цена | Чем определяется |
| --- | --- | --- |
| ROUTER | 0,03 мс | qadam не грузится |
| `tables` find / update, `store`, ответ вебхука | **25–70 мс** | сам запрос |
| **CODE-шаг** | **~74 мс** | спавн процесса ([#412](https://github.com/aiqadam/qadam-flow/issues/412) item 2, **не чинился**) |
| первый вызов каждого qadam'а в прогоне | +150 мс к первому шагу | однократный поиск пакета, дальше из кэша |
| отправка сообщения Bot API | 0,7–0,9 с | сеть до Telegram, платформа ни при чём |

**Замер делать только по прогретому инстансу.** Кэш из #417 живёт в процессе
воркера: после `ap_lock_and_publish` он холодный, и первое обращение к каждому
qadam'у снова платит обход. Сразу после публикации трёх флоу визард показал
21,9 с, через четыре минуты — 9,5 с (W24, 2026-09-13). Это не регрессия, это
непрогретый кэш; принимать первый прогон за результат нельзя.

**Историческая справка, важная для чтения старых замеров.** До 2026-09-13
каждый PIECE-шаг нёс **344–636 мс**: `qadamLoader.getQadamPath` не кэшировался

...
### Фиксированная цена PIECE-шага — 344–636 мс (дефект движка, не наша схема)

Добавлено 2026-09-13 по [qadam-flow#417](https://github.com/aiqadam/qadam-flow/issues/417)
(P0, открыт). `qadamLoader.getQadamPath` не кэширован ни на одном уровне: на
**каждом** PIECE-шаге движок рекурсивно обходит 238 папок `dist`, читая и
парся их `package.json`, пока не совпадёт имя. Инструментированный замер
апстрима на реальных прогонах:

| Шаг | Загрузка qadam'а | Своя работа | Итого |
| --- | --- | --- | --- |
| `qadam-webhook / return_response` | **414,6 мс** | **0,5 мс** | 416,5 мс |
| `qadam-tables / tables-find-records` | **592,2 мс** | 90,3 мс | 684,6 мс |

Подтверждается нашими прогонами независимо: ответ вебхука `checkin-api/step_18`
— 382–393 мс, чтения `tables` — 360–560 мс. `tables` и `subflows` стоят 234-м и
233-м из 238 в обходе, то есть у нас **худший случай по двум самым частым
шагам**.

**Что из этого следует при проектировании флоу:**

- считать надо **PIECE-шаги, а не чтения**: отправка сообщения, чтение,
  запись и ответ вебхука несут один и тот же налог;
- CODE-шаг (~72 мс) и ROUTER (0,03 мс) его не платят — CODE дорог по другой
  причине ([#412](https://github.com/aiqadam/qadam-flow/issues/412), item 2:
  спавн процесса);
- цена чтения `tables` **почти не зависит от объёма выборки** (30 строк — 472 мс,
  3 строки — 531 мс), потому что 80–85 % времени — это загрузка qadam'а, а не
  запрос. Отсюда правило «прочитать надмножество один раз и отфильтровать в
  CODE» — и его цена: всё прочитанное дословно уезжает в лог прогона
  ([Q31](OPEN-QUESTIONS.md#q31));
- **числа устареют, когда #417 починят.** Оценка эффекта фикса на наши флоу и
  порядок работ — [ADR-0009](adr/0009-hot-path-latency-budget-and-order.md).

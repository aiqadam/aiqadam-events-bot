...
Все три шага живут в одном месте — флоу [`fn-hmac-init-data`](../flows/fn-hmac-init-data.md),
вызываемом через `callFlow` из `checkin-api` и `my-qr-api`. Дублирования
между флоу больше нет, поэтому таблицу «кто встраивает копию» вести не для
чего: копия ровно одна, и она же — код ниже. Это заменило прежнюю схему
(до W26), где три шага цепочки были продублированы отдельно в каждом
конечном флоу и требовали побайтовой сверки друг с другом при каждом ревью.

...
Код живёт в [`fn-parse-start`](../flows/fn-parse-start.md), вызываемом через
`callFlow` из `tg-router` и `checkin-api`. Дублирования между вызывающими
больше нет — обе стороны получают решение из одного места, вариант «с
конвертом» (`{status, data}`) исчез вместе со старым `tg-router`.

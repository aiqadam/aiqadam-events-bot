...
|---|---|---|---|
| [`fn-sign-qr`](../flows/fn-sign-qr.md) | `step_1` | **подпись** (каноническая копия) | вызывается через `callFlow` из `my-qr-api` |
| [`fn-verify-qr`](../flows/fn-verify-qr.md) | `step_1` | проверка — **пересчитывает HMAC инлайн**, не зовёт `fn-sign-qr` | сознательное исключение из ADR-0015 п. 5 (латентность одного лишнего хопа на каждый скан); вызывается из `checkin-api` |

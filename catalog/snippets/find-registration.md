...
никто, кроме единственного канонического флоу.**

Код живёт в [`fn-find-registration`](../flows/fn-find-registration.md),
вызываемом через `callFlow` из `checkin-api` и `my-qr-api` — оба ищут ровно
одну регистрацию по `(event_id, telegram_id)`, известным из проверенной
подписи QR, и это ровно то, для чего эта функция написана. Перечитывание
после CAS (W20) было особенностью удалённого `registration`, снятой вместе
с ним (см. `checkin-api.md`: IDM-2 теперь через атомарный `only_if`, без CAS).
`reg-start` эту функцию не вызывает — ему нужен не поиск своей регистрации
по проверенной подписи, а список всех регистраций события (для подсчёта
занятых мест) и своя строка среди них; это читается напрямую
`tables-find-records`, без `fn-find-registration`.

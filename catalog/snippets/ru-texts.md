...
**W26 (2026-09-13) использует более лёгкий вариант этого эталона, не
байт-в-байт этот файл.** Отдельного шага-резолвера (с `vars`/`varsByKey`/
`missing`/`fellBack`) в новых флоу нет: каждый CODE-шаг, формирующий
сообщение, сам принимает `texts` (и, где нужно, простую подстановку `{name}`)
и сразу отдаёт готовый `text` — резолвер и бизнес-логика не разнесены по
разным шагам, как раньше. Инвариант («текст — во входе `texts`, не
...
| Флоу | Шаги | Вариант |
|---|---|---|
| `checkin-api` (удалён при очистке) | `step_9`, `step_10`, `step_31`, `step_43`, `step_44`, `step_46` | плоский |
| `my-qr-api` (удалён при очистке) | `step_25`, `step_27` | плоский |
| `registration` (удалён при очистке) | `step_99` | плоский |

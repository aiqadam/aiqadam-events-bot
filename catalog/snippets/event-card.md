...
**Встраивают:** никто в W26. Удалённый `registration` был единственным
потребителем этого четырёхшагового эталона (extract → fmt батчем → тексты →
assemble). [`reg-start`](../flows/reg-start.md) (`step_9`, «build event card»)
строит ту же карточку одним CODE-шагом — без батч-вызова `fmt-time` и без
отдельного шага-резолвера текстов (см. пересмотр [`fmt-time.md`](fmt-time.md)
и [`ru-texts.md`](ru-texts.md) после ADR-0014/W26). Карточка не использует
этот эталон побайтово — см. заметку в `reg-start.md`.

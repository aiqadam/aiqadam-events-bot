...
> ## ⚠️ Потребители не существуют, сами переменные не проверены
>
> Инстанс `events-dev` очищен 13.09.2026: флоу и таблиц нет. Значит **таблицы
> «кто их читает» и аудит `{{VAR}}` ниже описывают флоу, которых больше нет** —
> это история пакета W18, а не текущее состояние.
>
> Сами Variables очистку, скорее всего, пережили (удалялись флоу и таблицы),
> но **проверено это не было**: MCP их не перечисляет, смотреть надо в UI.
> Первый пункт [W26](../docs/work/W26-rebuild-on-one-touch.md) — убедиться,
> что `QR_SIGNING_KEY`, `BOT_TOKEN`, `BOT_USERNAME` и `MINIAPP_URL` на месте.
>
> Что остаётся в силе независимо от очистки: имена, назначение, правило
> `{{variables['NAME']}}` против молча пустой короткой формы `{{NAME}}`
> и вывод, что короткую форму ловит `ap_validate_flow` — REST для этого не нужен.
...
**Результат: ни одного вхождения короткой формы.** Инцидент не объявлялся,
`QR_SIGNING_KEY` не ротировался.

**Область аудита названа точно** (замечание второго круга ревью W18): проверены
**пять флоу проекта** — те, что описаны в [overview](overview.md). Во время
пакета в `events-dev` появился **шестой**, `bench-421-loop-code-final`, к
проекту не относящийся ([Q34](../docs/OPEN-QUESTIONS.md#q34)); он проверен
отдельно ревьюером и владельцем — ссылок на переменные в нём нет. Формулировка
«все пять флоу» утверждала бы полноту, которой у неё не было.
...
### Снимок аудита (пять флоу проекта, 2026-09-13)

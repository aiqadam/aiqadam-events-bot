...
## Токен бота как значение шага (W2)

`fn-verify-init-data` подставляет токен в `text` у `crypto / hmac-signature`
...
цепочки). Проверено прогоном на инстансе 2026-09-08. Секрет при этом остаётся
в платформе — в репозитории только externalId connection.

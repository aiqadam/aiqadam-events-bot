...
- **ПД**: `answer` (свободный текст) и `display_name` лежат в таблице и
  выгружаются владельцем. Следствие — самоудаление аккаунта обязано чистить
  и эти строки; на момент W103 `reg-api/delete_account` этого ещё не делает,
  это названный хвост пакета.

...
- Попытки пользователя удаляются самоудалением аккаунта (см. `quiz_answers`);
  отдельного действия в `reg-api` для этой таблицы нет — строки викторины
  удаляются вместе с ответами, это хвост W103.

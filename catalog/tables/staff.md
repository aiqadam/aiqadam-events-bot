...
- **Назначение**: глобальные права организаторов, ограниченные чаптером
  ...
  строка в `staff` даёт право **создавать и править** события своего чаптера.
...
- **Ведёт человек в UI платформы.** Две строки, обе `chapter_id = 1`:
  `322876545` (`return_void_0`, заведена [W32](../../docs/work/W32-owners-initdata.md))
  и `52128246` (`Viktor Drukker`, `@ViktorDrukker`, решением владельца 2026-09-22,
  пакет `owner`).
- Не путать с [`event_staff`](event_staff.md) — там контролёры конкретного
  события (per-event), здесь глобальный список организаторов (per-chapter).

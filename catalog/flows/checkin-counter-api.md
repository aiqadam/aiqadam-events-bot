...
| step_3 (valid) | `tables-find-records event_staff` | `(event_id, telegram_id контролёра)` + `revoked_at not_exists` — гейт по конкретному событию |
| step_4 (valid) | `tables-find-records registrations` | строки события, проекция `status` + `checked_in_at`, `limit: 500` — только для подсчёта, без ПД |

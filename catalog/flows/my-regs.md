...
| step_2 | `tables-find-records registrations` | мои регистрации (`telegram_id eq …`, проекция `event_id`, `status`, `checked_in_at`), `limit 50` | `{{trigger['output'].data.telegramId}}` |
...
- **Проекция `step_3` обязана включать `status`**: постфильтр
  `status = published` в коде читает его же, без колонки в выдаче фильтр
  выкидывает все строки молча, а не падает (найдено прогоном при сборке).
- **Тексты — во входе `texts`** (ADR-0014); значения сверены с `i18n/ru.json`.
- Дубли на `(event_id, telegram_id)` схлопываются до первой строки —
  чтение показывает одну запись на ивент.

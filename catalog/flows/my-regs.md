...
- **Триггер**: `@aiqadam/qadam-subflows : callableFlow` — вызывается из `tg-router`
  на колбэк `myreg:list` из меню (W34). `callbackData`
  используется только для ack (`answer_callback_query`); список рендерится
  в обоих случаях (ADR-0015: касание = один вход)

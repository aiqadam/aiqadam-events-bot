...
- **Ссылка «Открыть на карте» в карточке (`step_4`) — только при валидных
  координатах** (`str(ev.lat) !== '' && str(ev.lon) !== '' && isFinite &&
  lat∈[-90,90] && lon∈[-180,180] && !(lat === 0 && lon === 0)`): `Number('') === 0`
  иначе давал ссылку на точку 0,0 в Атлантике. Guard завёл W76 (#128), проверку
...
- **`finish_lite` не переспрашивает согласие на рассылку (W60).** Ветка
  ...
  W60 нашёл дубль литерала `'Открыть билет'`, скопированного из
  `reg-consent-mkt/step_5`, — заведён ключ, `reg-consent-mkt` не тронут,
  цена правки его отдельного литерала — новая публикация без изменения
  поведения — принята отдельно, не в этом пакете).
- **Отказ (`declined`) различает диплинк и голый `/start` по `eventId`**:
  ...
  W73 (#125): `declined_no_link` и «Подробнее» (`onb.details`) больше не
  обещают «напишите нам/боту снова» — первый ведёт к `/start`, второй
  указывает на таб «Профиль» (отзыв согласия и удаление данных, ADR-0039);
  текст «откройте ту же ссылку ещё раз» не менялся — он и так работает.
- **Финальная карточка (`finish`/`finish_lite`) несёт факты события** (W76):
  дата, адрес и «Открыть на карте» — теми же полями и той же проверкой
  координат, что карточка события; пустые координаты и `(0,0)` — не гео

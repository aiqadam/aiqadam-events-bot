...
- **Ссылка «Открыть на карте» в карточке (`step_4`) — только при непустых
  координатах** (`str(ev.lat) !== '' && str(ev.lon) !== '' && !(lat === 0 && lon === 0)`):
  `Number('') === 0` иначе давал ссылку на точку 0,0 в Атлантике. Та же проверка
  в `reg-start`/`reg-consent-mkt` (W72, #124).

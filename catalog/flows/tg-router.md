...
- ⚠️ **Черновик расходится с опубликованной версией на `step_18`** (с 2026-09-12,
  W19). Этот файл описывает **опубликованную** версию — она работает и не
  тронута. В черновике `step_18` пересобран заново: правка одного `displayName`
  вызвала перевалидацию против новой версии `@aiqadam/qadam-subflows` и обнажила
  отсутствие ставшего обязательным пропа `executionMode` (его добавил
  [#363](https://github.com/aiqadam/qadam-flow/issues/363) уже после того, как
  W5 написал этот вход). Восстановлено по контракту `registration/step_1`,
  `executionMode = queue` — как решил W17. `ap_validate_flow` → 21/21.
  **Кто будет публиковать `tg-router` следующим — сначала читает
  [журнал W19](../../docs/work/W19-platform-primitives-revision.md).**
  `step_20` — в том же состоянии, но ещё не тронут и числится валидным.
- **Шаги `tables` и `store` этого флоу сейчас не редактируются через MCP** —
  ...
  при этом валиден и работает; отказ только на запись. Новые шаги создаются.

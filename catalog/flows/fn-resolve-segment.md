...
- **`users` читается двумя фильтрованными выборками, а не целиком.** Первая версия
  ...
  **Поправка 2026-09-12 (W19):** здесь стояло «у найденных строк всё равно
  логируются все колонки — проекции колонок у `tables` qadam нет». Это **более
  неверно**: проп называется `columns`, он появился с обновлением образа и
  применён к `step_4`/`step_7` (см. таблицу шагов выше). Остаточного риска по
  этим двум чтениям нет ([Q17](../../docs/OPEN-QUESTIONS.md#q17),

...
- Проверено `ap_test_flow` (photo/skip/невалидный текст) и интеграционно
  через `tg-router` (`route: wiz_photo` на реальном фото-апдейте, прогон
  `OeYdh1MyrrOii97K96XSC`).

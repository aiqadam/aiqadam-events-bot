...
- **Триггер**: `@aiqadam/qadam-subflows : callableFlow` — вызывается из
  ...
  «общие чтения». Обработчики регистрации его **не** зовут:
  `reg-start` и `reg-api` читают `registrations` напрямую,
  потому что им нужна выборка по событию или по участнику целиком, а не

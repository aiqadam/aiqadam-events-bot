...
| step_1 | `tables-find-records staff` | строка `staff` пользователя (limit 1) — видимость кнопок овнера (W50; вердикт: создание из чата — сразу форма `#/manage/new`; W51: +«Панель администратора» → `#/manage`) |
| step_2 | `tables-find-records event_staff` | все staff-строки пользователя (limit 50) |
| step_3 | `tables-find-records events` | опубликованные события (status = `published`, limit 50) |
| step_4 | CODE «render menu» | сборка кнопок (W52): гость — 1 кнопка; организатор — [`Панель администратора` → `#/manage`, `События`, `Новое событие`]; контролёр — тот же 1 экран, свой текст; кнопки сканера в чате нет |
...
- **Кнопки `web_app` собираются из `MINIAPP_URL`**: `#/events` (все роли —
  ...
  Кнопки сканера в чате нет: сканер — рядом с событием в Mini App по правам
  плюс кнопка «Открыть сканер» на конкретное событие в accept-карточке
...
- **Staff-фильтр — defense-in-depth в CODE step_4**, не в tables-запросе: шаг_2
  ...
  не выбирает событие для кнопки — только текст и `role`; отозванный контролёр
...
- **Порядок кнопок фиксирован, по одной в ряд**: гость — [`События`];
  ...
  (кнопки овнера).
  `Панель администратора` ведёт на список `#/manage` (вход в правку без команд,
...
- **«Мои события» заменены «Новое событие» обратно (W50, вердикт W49)** —

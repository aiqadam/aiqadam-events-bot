...
- **Кнопка «Викторина» — только в окне (W103, ADR-0041).** Пока
  `now ∈ [starts_at, ends_at]` хотя бы одной строки `quizzes`, кнопку видят
  **все роли**: гость и контролёр — первой строкой, организатор — после своих
  кнопок («Панель администратора» у него остаётся первой, W52). Вне окна
  ...
  `has_profile`. Различающие прогоны: гостевой профиль — в `reply_markup`

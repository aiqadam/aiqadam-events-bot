...
- **Порядок кнопок фиксирован**: гостевые (`Ивенты` — `web_app` на `#/events`,
  `Мои билеты` — `web_app` на `#/events?tab=mine`) → организатор (`Создать
  ивент`, `web_app #/manage`) → контролёр (`Сканер`, `web_app
...
- **«Мои ивенты» не рисуются до W13** — вместо неё гостю дана кнопка
  «Мои билеты» на первый таб каталога (W43).

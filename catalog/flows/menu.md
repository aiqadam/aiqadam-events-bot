...
- **Кнопки `web_app` собираются из `MINIAPP_URL`**: `#/events` (все роли —
  каталог и «Мои билеты» — один экран, открывается первым табом) и
...
- **Порядок кнопок фиксирован, по одной в ряд**: гость — [`Ивенты`];
  организатор — [`Ивенты`, `Создать ивент`]; контролёр — [`Ивенты`] со своим
  текстом; двойная роль — объединение (кнопки овнера). Гость → 1 кнопка,
  организатор → 2. Общего заголовка «Что дальше?» нет: у каждой роли свой

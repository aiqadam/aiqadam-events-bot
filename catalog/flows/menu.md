...
- **Порядок кнопок фиксирован**: гостевые (`Ивенты` — `web_app` на `#/events`,
  `myreg:list`) → организатор (`Создать ивент`, `web_app #/manage`) → контролёр
  (`Сканер`, `web_app #/scan?event_id=`). Организатор+контролёр → 4 кнопки.
  Контролёр без будущих ивентов → без кнопки сканера (3 кнопки). Гость → 2 кнопки.
  Каталог ивентов — экран (ADR-0023), а не чат-лист: колбэков `ev:list:*`
  у бота больше нет (W38).

...
- **Кнопка «Викторина» — только в окне (W103, ADR-0041).** Гость и контролёр
  видят её первой, пока `now ∈ [starts_at, ends_at]` хотя бы одной строки
  `quizzes`; у организатора кнопки нет (его набор не меняется). Вне окна
  кнопки нет — мёртвых кнопок не держим. Окно проверяется в CODE `step_11`
  (диапазон по DATE не работает, Q15); чтение `quizzes` — `step_14` в ветке
  `has_profile`. Различающий прогон: гостевой профиль (не staff) — в

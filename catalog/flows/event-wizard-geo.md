...
- **Гео принимается только сообщением-локацией** (`message.location`), не
  текстом с координатами — закрывает пункт чек-листа W11 «гео принимается
  сообщением-локацией».
- Отдельный флоу по той же причине, что и `event-wizard-photo`: другая
  механика входа (гео-сообщение), не текст — ADR-0016.
- Проверено `ap_test_flow` (location/skip/невалидный текст) и интеграционно
  через `tg-router` (`route: wiz_geo` на реальном гео-апдейте, прогон
  `DxSoEcujIrNUfdUQQSQc8`).

...
- **Права здесь не решаются намеренно.** Кнопка на чужой `event_id` откроет
  ...
  проверяется `staff` и чаптер ([ADR-0024](../../docs/adr/0024-staff-by-chapter-event-staff-checkin.md)).
  Проверять дважды значило бы завести второе место, где правило может разойтись с первым.

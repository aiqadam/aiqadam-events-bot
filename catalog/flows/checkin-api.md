...
пустым `revoked_at`. Без `eventIdValid`/`initDataValid`, либо если ключ не
проходит проверку **формы** (`^[A-Za-z0-9_]{1,12}$` для `event_id`,
`^[0-9]{1,16}$` для `telegram_id`) — отказ, не выдача (fail-closed). Проверка
формы, а не непустоты — правка второго круга ревью, см. раздел ниже: она
закрывает и пустое значение, и сентинел `-`. Наружу добавлены
`rowsRead`/`rowsMatched`/`keysOk`: расхождение между первыми двумя означает,
что фильтр чтения отработал шире, чем должен.

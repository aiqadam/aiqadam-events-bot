...
- **`event_staff` фильтруется по `(event_id, telegram_id, revoked_at not_exists)`
  ...
  **Важно (правка 2026-09-11):** здесь раньше стояло утверждение, что
  «пользователь вообще где-то staff» получить физически невозможно, потому что
  `event_id` — член фильтра, а не постфильтр в коде. **Это утверждение неверно.**
  [qadam-flow#382](https://github.com/aiqadam/qadam-flow/issues/382): если форма
  `filters` перестаёт распознаваться, `tables-find-records` молча возвращает
  **все** строки таблицы — тогда `step_6` (`isStaff = records.length > 0`) даёт
  `true` для любого пользователя с валидным `initData`. Постфильтра, который
  это поймал бы, в `step_6` сегодня нет. Права проверяются правильно ровно
  до тех пор, пока форма фильтра остаётся верной, и ничего на платформе этого
  не гарантирует — см. [Q25](../../docs/OPEN-QUESTIONS.md#q25), митигация —
  пакет [W18](../../docs/BACKLOG.md).

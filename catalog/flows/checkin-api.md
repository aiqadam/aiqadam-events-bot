...
- **Контракт ответа согласован с уже задеплоенным `miniapp/index.html`
  (сканер)**: `{status, text}` плоско в теле, `status = invalid_init_data`
  на 401 (не `unauthorized` — так изначально ждал клиентский JS, поправлено
  после того, как статическая страница получила ответ, которого не
  распознавала).
- **Проверено живым прогоном 2026-09-13 (владелец, реальные Telegram-клиенты
  `@return_void_0` и `@binali_beelab`), а не только синтетикой.** Два скана
  реальной подписи (`hashValid:true`, `QR_SIGNING_KEY` не менялся — сигнатура
  `aIxmwbnzb_` для `(demo, 322876545)` совпала с историческим значением из
  `catalog/snippets/hmac-qr.md` до очистки инстанса): первый → `ok`,
  `checked_in_at` записан атомарно (`only_if`); второй → `already`, время
  «18:41» (Tashkent из `13:41:37Z`, UTC+5 подтверждён), `checked_in_at`
  **не переписан** — IDM-2 и STF-2 (реальная запись `event_staff` по
  `(event_id, telegram_id)` найдена) подтверждены на боевых данных, не
  только на синтетическом стенде. Прогоны — `fyfRgcnKbo7SGKGOAEDFw` (ok),
  `4M8AGK4fyOpVqv9XsyybU` (already). Ранее агент проверял только
  `invalid_init_data` (401, мусорный `initData`) и атомарность записи
  отдельным стендом — это закрывает STF-2 из чек-листа W26 полностью.
- **Найден и исправлен блокер, не в самом флоу**: задеплоенная
  `miniapp/index.html` указывала на **старый** `flowId` `checkin-api`
  (до-очистки, мёртвый) — GitHub Pages раздавал её без изменений, потому
  что W26 шёл веткой, а Pages деплоится только с `main`. Исправлено PR
  [#35](https://github.com/aiqadam/aiqadam-events-bot/pull/35), смёржен
  в `main` по разрешению владельца в момент живого теста.

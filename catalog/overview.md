...
|---|---|---|
| Флоу | **10** — пять `fn-*`, четыре касания регистрации, `tg-router`; `checkin-api`, `my-qr-api` ещё нет | `ap_list_flows`, карточки в [flows/](flows/) |
...
Пять `fn-*`, четыре касания регистрации и `tg-router` собраны, проверены
позитивными/негативными прогонами (с реальной доставкой в Telegram владельца)
и опубликованы — карточки в [flows/](flows/). Осталось собрать
`checkin-api`/`my-qr-api`. Старые карточки этих двух флоу в [flows/](flows/)

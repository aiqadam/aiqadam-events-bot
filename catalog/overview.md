...
|---|---|---|
| Флоу | **0** — сборка ещё не начиналась | `ap_list_flows` → `{"flows":[],"count":0}` |
...
Флоу ещё предстоит собрать заново по ADR-0015. Старые карточки flows/ в
[flows/](flows/) описывают запрещённую схему и **пересобирать по ним нельзя**;
таблицы уже пересобраны, их карточки обновлены и достоверны.

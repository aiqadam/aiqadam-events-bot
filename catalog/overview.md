...
- **Уровень коррекции — H**, умолчание `qrcode.js`. Бренд для печатных стендов
  предписывает Q; H даёт больше избыточности, а не меньше, поэтому расхождение
  идёт в сторону запаса. Переносить ли требование бренда на экран — вопрос
  к владельцу, не решённый этим пакетом.

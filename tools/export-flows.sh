...
# на несуществующей. Доказательство, что снимок с опубликованной версии, —
# сверка version.id с publishedVersionId ЗДЕСЬ, при выгрузке, плюс запись
# в _manifest.json. Сам по себе state: LOCKED этого не доказывает:
# замороженной бывает и неопубликованная версия.
#
...
# Всё остальное остаётся как есть. `state` сохранён намеренно, но как
# признак, а не доказательство: он показывает, что версия заморожена,
# а какая именно — говорит _manifest.json.
NORMALIZE='
  .version
  | walk(if type == "object"

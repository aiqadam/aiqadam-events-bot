...
PROTO.protoDict = {
  ...
  // proto.broadcast_chat_hint и proto.open_chat переехали в ru.json
  // (тексты побуквенно те же, места вызовов — через T()).
  'proto.deep_link_note': 'переход по ссылке регистрации',
  'proto.staff_invite_note': 'переход по ссылке-инвайту',
  // W10: подсказка под одноразовой ссылкой контролёра в табе «Контролёры»
  // (в продукте, вероятно, станет `staff.invite.hint`).
  'proto.staff_invite_hint': 'Действует 24 часа и только один раз.',
  ...
  // Формат участия (вердикт 2026-09-18), контролёры (W55): ключи и тексты —
  // из ru.json (manage.geo.*, manage.staff.*), места вызовов переведены.
  ...
  // онбординг C (ADR-0032) и профиль гостя: тексты написаны и сгенерированы
  // из i18n/ru.json (W50) — здесь их больше нет. Мок-подстановки имен
  // ({name}, {profile}) живут на местах вызовов в scenarios.js.
};
